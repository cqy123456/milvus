...
class BinlogIndexTest : public ::testing::TestWithParam<Param> {
    void
    SetUp() override {
        std::tie(data_type, metric_type, index_type, dense_vec_intermin_index_type) =
            GetParam();
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        std::make_tuple(DataType::VECTOR_FLOAT,
                        ...
                        knowhere::IndexEnum::INDEX_FAISS_IVFFLAT_CC), // without field data 
        std::make_tuple(DataType::VECTOR_FLOAT,
                        knowhere::metric::L2,
                        knowhere::IndexEnum::INDEX_FAISS_IVFFLAT,
                        knowhere::IndexEnum::INDEX_FAISS_SCANN_DVR), // with field data 
        std::make_tuple(DataType::VECTOR_SPARSE_FLOAT,
                        knowhere::metric::IP,
                        knowhere::IndexEnum::INDEX_SPARSE_INVERTED_INDEX,  // with field data 
                        std::nullopt), 
        std::make_tuple(DataType::VECTOR_SPARSE_FLOAT,
                        knowhere::metric::IP,
                        knowhere::IndexEnum::INDEX_SPARSE_WAND, // with field data 
                        std::nullopt)));
...
TEST_P(BinlogIndexTest, AccuracyWithLoadFieldData) {
    ...
    if (dense_vec_intermin_index_type.has_value()) {
         segcore_config.set_dense_vector_intermin_index_type(dense_vec_intermin_index_type.value());
    }
    ...
    if (dense_vec_intermin_index_type.has_value() && dense_vec_intermin_index_type.value() == knowhere::IndexEnum::INDEX_FAISS_IVFFLAT_CC) {
