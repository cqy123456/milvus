...
ScalarIndexSort<T>::Serialize(const Config& config) {
    ...
    BinarySet res_set;
    res_set.Append("index_data", std::move(index_data), index_data_size);
    res_set.Append("index_length", std::move(index_length), sizeof(size_t));
...
ScalarIndexSort<T>::Load(const Config& config) {
    ...
    for (auto& [key, data] : index_datas) {
        ...
        binary_set.Append(key, std::move(buf), size);
    }
