...
Slice(const std::string& prefix,
    ...
    for (int64_t i = 0; i < data_size; ++slice_num) {
        ...
        binarySet.Append(
            GenSlicedFileName(prefix, slice_num), std::move(slice_i), ri - i);
        i = ri;
    }
    ret[NAME] = prefix;
...
Assemble(BinarySet& binarySet) {
    ...
    for (auto& item : meta_data[META]) {
        ...
        binarySet.Append(prefix, std::move(p_data), total_len);
    }
}
...
AppendSliceMeta(BinarySet& binarySet, const Config& meta_info) {
    ...
    binarySet.Append(INDEX_FILE_SLICE_META, std::move(meta_data), meta_len + 1);
}
