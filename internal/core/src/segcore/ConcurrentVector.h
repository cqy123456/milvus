 ...
 public:
    ...
    get_span(int64_t chunk_id) const {
        ...
        if constexpr (is_type_entire_row) {
            return Span<TraitType>(static_cast<Type*>(chunk_data), chunk_size);
    ...
    get_chunk_data(ssize_t chunk_index) const override {
        if constexpr (!IsVariableType<Type>) {
            ...
            PanicInfo(DataTypeInvalid,
                      fmt::format("get_chunk_data only fixed size data type"));
        }
    }
    ...
    get_element(ssize_t element_index) const {
        if constexpr (!IsVariableType<Type>) {
            ...
            PanicInfo(DataTypeInvalid,
                      fmt::format("get_element only fixed size data type"));
        }
    }

    Type
    operator[](ssize_t element_index) const {
        // operator[] in concurrentvector only use as rvalue
        AssertInfo(
            elements_per_row_ == 1,
            fmt::format(
        ...
        auto chunk_id = element_index / size_per_chunk_;
        auto chunk_offset = element_index % size_per_chunk_;
        return chunks_ptr_->get_element(chunk_id, chunk_offset);
    }
