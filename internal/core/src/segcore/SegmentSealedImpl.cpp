...
SegmentSealedImpl::generate_binlog_index(const FieldId field_id) {
    ...
    try {
        ...
        index::IndexBasePtr vec_index = std::make_unique<index::VectorMemIndex>(
            field_binlog_config->GetIndexType(),
            index_metric,
            knowhere::Version::GetCurrentVersion().VersionNumber());
        vec_index->BuildWithDataset(dataset, build_config);
        // check again, in case vector index has been loaded.
        if (enable_binlog_index()) {
            std::unique_lock lck(mutex_);
            vector_indexings_.append_field_indexing(
            ...
            LOG_SEGCORE_INFO_ << "replace binlog with binlog index in segment("
                              << this->get_segment_id() << "), field("
                              << field_id.get() << ").";
        }
        return true;
    } catch (std::exception& e) {
        LOG_SEGCORE_WARNING_ << "fail to generate binlog index and continue "
                                "use raw data, because"
                             << e.what();
        return false;
    }
}
