...
struct MappState {
 ...
 private:
    ...
    std::atomic<uint64_t> mmapp_file_counter;
    uint64_t fix_mmap_file_size;
};
using MappStatePtr = std::shared_ptr<MappState>;

/**
 * @brief MappEntry handle all memory mapping in one tmp file
 */
struct MappEntry {
 public:
    ...
    MappEntry(const std::string& file_name,
...
using MappEntryPtr = std::shared_ptr<MappEntry>;
...
class FixedSizeMmapEntryQueue {
 public:
    FixedSizeMmapEntryQueue(const MappStatePtr& entry_meta_ptr)
 ...
 private:
    ...
    MappStatePtr mmap_entry_meta_ptr_;
    std::queue<MappEntryPtr> queuing_entries_;
...
class GrowingMmapChunkManager {
 ...
 private:
    MappStatePtr mmap_entry_meta_ptr_ = nullptr;
    ...
    std::unordered_map<uint64_t, std::vector<MappEntryPtr>> activate_entries_;
